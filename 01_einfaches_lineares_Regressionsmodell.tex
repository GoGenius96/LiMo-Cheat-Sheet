...
\begin{tikzpicture}
    \node [mybox] (box){%
        ...
        \begin{itemize}
            ...
            Alternativ: Wenn $x_i$ um 1\% steigt, dann steigt $Y_i$ im Erwartungswert um $100\cdot(e^{\gb1 log(1.01)} - 1)$\%.
...
\node [mybox] (box){%
    \begin{minipage}{0.3\textwidth}
    $R^2$ ist abhängig von $X$. Das heißt über mehrere Studien hinweg, die das
    gleiche messen, ist $R^2$ nur vergleichbar, wenn auch $X$ vergleichbar ist.
    Je sichererer wir mit unserem Schätzer sein wollen, desto höher sollten wir
    die Varianz von $X$ einstellen. Gegeben, dass der Zusammenhang tatsächlich
    linear ist, würde eine höhere Varianz von $X$ zu einer geringeren Varianz
    von $\hbe1$ führen.\\
...
\begin{tikzpicture}
    \node [mybox] (box){%
        ...
        \begin{align}
            \bY &= \bX \bbeta + \beps\\
            \E(\beps) &= \bnull\\
            \V(\eps_i) &= \ssd , \text{ für alle } i = 1,\dots,n\\
            \eps_i & \text{ sind paarweise unabhängig voneinander}\\
            \eps_i &\sim \Ncal(0,\ssd), \text{ für alle } i = 1,\dots,n
        \end{align}

        Die folgende Tabelle gibt an, welche Annahmen für die jeweiligen
        Schätzer, Eigenschaften, Größen etc. benötigt werden.

        \begin{tabular}{|l|l|l|l|l|l|}
            \hline
            & (1) & (2) & (3) & (4) & (5)\\
            \hline
            KQ-Schätzer & $\checkmark$ & & & &\\
            \hline
            ML-Schätzer & $\checkmark$ & $\checkmark$ & $\checkmark$ & $\checkmark$ & $\checkmark$\\
            \hline
            $\E(\hbbeta) = \bbeta$ & $\checkmark$ & $\checkmark$ & & & \\
            \hline
            $\hbbeta \sim \Ncal(\bbeta, \V(\hbbeta))$ & $\checkmark$ & $\checkmark$ & $\checkmark$ & $\checkmark$ & ($\checkmark$)\\
            \hline
            Konfidenzintervalle & $\checkmark$ & $\checkmark$ & $\checkmark$ & $\checkmark$ & ($\checkmark$)\\
            \hline
            Prädiktionsintervalle & $\checkmark$ & $\checkmark$ & $\checkmark$ & $\checkmark$ & $\checkmark$\\
            \hline
        \end{tabular}

        ($\checkmark$) bedeutet, dass die Annahme nicht benötigt wird, wenn der Stichprobenumfang $n$ groß genug ist.
        \end{minipage}
    };
%------------ Annahmen des linearen Regressionsmodells Header ---------------------
\node[fill = purple, text=white, font=\bfseries, right=10pt] at (box.north west)
{Annahmen des linearen Regressionsmodells};
\end{tikzpicture}
