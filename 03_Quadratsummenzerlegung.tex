...
\begin{tikzpicture}
    \node [mybox] (box){%
        ...
        zu testen. Wir lehnen $H_0^O$ ab, wenn \\$F_0 > F_{1-\alpha}(p'-1, n-p')$.
        \end{minipage}
    };
%------------ Overall-Test Header ---------------------
\node[fill = black, text=white, font=\bfseries, right=10pt] at (box.north west) 
...
\begin{tikzpicture}
    \node [mybox] (box){%
        ...
        $$R(\gb{k} | \gb{0}, \dots, \gb{{k-1}}) = SSE(M_{k}) - SSE(M_{{k-1}})$$ für $k = 1, \dots, p'$.
...
\begin{tikzpicture}
    \node [mybox] (box){%
        ...
        Wir konstruieren das \tc{Prognoseintervall} für $\E(Y_{n+1}) = {\mu}_{n+1}$ zum Niveau $1 - \alpha$ als
